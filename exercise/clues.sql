...
SELECT name FROM countries WHERE population = (SELECT MIN(population) FROM countries WHERE continent = 'Europe');
...
SELECT * FROM countrylanguages WHERE language='Italian' AND isofficial='T' AND percentage=100;
...
SELECT ci.name FROM cities ci JOIN countries co ON ci.countrycode = co.code WHERE co.continent = 'South America' AND ci.name ILIKE 'Serra%';
SELECT co.name AS country FROM cities ci JOIN countries co ON ci.countrycode = co.code WHERE ci.name = 'Serra';
