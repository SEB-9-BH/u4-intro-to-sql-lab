...
-- Write SQL query here
SELECT ct.name AS city, co.name AS country
FROM cities ct
JOIN countries co ON ct.countrycode = co.code
WHERE co.name IN (
    SELECT c.name
    FROM countries c
    JOIN cities ct ON c.code = ct.countrycode
...
WHERE co.continent = 'South America'
  AND ct.name LIKE 'Serra%'
  AND ct.name <> 'Serravalle';
...
SELECT ct.name AS city, co.name AS country
FROM countries co
JOIN cities ct ON co.capital = ct.id
...
-- Write SQL query here:
SELECT ct.name AS city, co.name AS country, ct.population
FROM cities ct
JOIN countries co ON ct.countrycode = co.code
